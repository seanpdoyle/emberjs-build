...
function EmberBuild(opts) {
  var options = opts || {};

  this._init();

  this._additionalVendoredPackages = options.vendoredPackages || {};
  this._vendoredPackages = getVendoredPackages(this._additionalVendoredPackages);
  ...
  if (options.packages) {
    ...
    if (!this._skipTemplates) {
      this._trees.distTrees.push(this._buildTemplateCompilerTree, this._vendoredPackages);
    }
...
EmberBuild.prototype._init = function init() {
  ...
  this._additionalVendoredPackages = null;
  this._vendoredPackages           = null;
...
EmberBuild.prototype._enumeratePackages = function enumeratePackages() {
  ...
  var prodSourceTrees      = [];
  var vendoredPackages     = this._vendoredPackages;

  for (var packageName in packages) {
    var currentPackage = packages[packageName];

    currentPackage.trees = es6Package(packages, packageName, this._vendoredPackages);
...
EmberBuild.prototype.getDistTrees = function getDistTrees() {
  ...
  if (!buildConfig.isDevelopment) {
    ...
    if (!this._skipRuntime) {
      distTrees.push(this._buildRuntimeTree(this._packages, this._vendoredPackages));
    }
  }
